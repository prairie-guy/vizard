...
- **ROW** - Column to facet by rows (creates vertical small multiples)
- **COLUMN** or **COL** - Column to facet by columns (creates horizontal small multiples)
...
### Grouping & Layout
- **GROUP_TYPE** - For bar charts with multiple categorical dimensions: `grouped` (side-by-side) or `stacked` (vertical stacking)
...
**Faceted scatter plot (small multiples by rows):**
```
DATA gene_data.csv
PLOT scatter
X log2fc Y pvalue
...
**Faceted by rows and columns (grid):**
```
DATA expression.csv
PLOT bar
X gene Y count
