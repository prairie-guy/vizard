...
1. **Every Vizard invocation (EXECUTE IN THIS ORDER):**
   - **STEP 1 (MANDATORY):** Use Bash tool to read `.vizard_state.json` from current directory
     - If file doesn't exist, create it with default values
     - Defaults: ENGINE=altair, DF=polars, WIDTH=600, HEIGHT=400, FUNCTION=false, IMPORT=false, OUTPUT=display
   - **STEP 2 (MANDATORY):** Parse ALL CAPITALIZED keywords from user's specification
     - Include: SNAKE_CASE keywords (X_TITLE, Y_TITLE, X_LABEL_ORIENT, COLOR_SCHEME, THRESHOLD, etc.)
     - Exclude: Wrangling keywords (FILTER, SELECT, DROP, SORT, ADD, GROUP, RENAME, BIN, JOIN, STRING, CAST, PIVOT, UNPIVOT, UNIQUE, HEAD, CONCAT, DROP_NULLS, FILL_NULLS, IS_NULL, MAP, SAVE)
     - Extract keyword name and value from user input
   - **STEP 3 (MANDATORY):** Merge existing state with new keywords
     - existing_state + new_keywords = updated_state
     - New keywords override existing values
   - **STEP 4 (MANDATORY):** Use Bash tool to write updated state to `.vizard_state.json`
     - Write complete JSON with proper formatting
     - Ensure file is valid JSON
   - **STEP 5:** Generate Python visualization code using current state values

   **CRITICAL:** Steps 1-4 are NON-NEGOTIABLE. State persistence MUST happen before code generation.
   If you skip state management, the system will not function correctly.
...
### Chart Composition
- **LAYER** - Overlay chart layer on current chart (natural language description)
  - Examples: "text labels", "Least Square Line", "Horizontal line at mean of Y"
  - Uses Altair's `+` operator
  - Inherits X, Y, COLOR encodings from base chart unless overridden
  - Syntax: `|| LAYER <description>`
- **PLOT with position** - Position new chart relative to current chart
  - Positions: `above`, `below`, `left`, `right`
  - `above`/`below` uses `&` operator (vertical concatenation)
  - `left`/`right` uses `|` operator (horizontal concatenation)
  - Smart axis sharing: above/below shares X-axis, left/right shares Y-axis (when no explicit encodings)
  - Syntax: `|| PLOT <type> <encodings> <position>`
  - Example: `|| PLOT histogram X below`
...
### State Persistence - Concrete Examples

**Example 1: Axis Label Configuration**
...
X_LABEL_ORIENT horiz
PLOT scatter X Horsepower Y Miles_per_Gallon
```

**State file BEFORE execution:**
```json
{
  "ENGINE": "altair",
  "DF": "polars",
...
**State file AFTER execution (REQUIRED):**
```json
{
  "ENGINE": "altair",
  "DF": "polars",
...
Highlight where pvalue < THRESHOLD and abs(log2fc) > LOG2FC_CUTOFF
```

**State file AFTER (numeric values preserved):**
```json
{
  ...defaults...,
  "THRESHOLD": 0.05,
...
**Next cell - keywords are automatically available:**
```
%%cc
DATA genes2
PLOT scatter X log2fc Y pvalue
Highlight where pvalue < THRESHOLD
```
No need to re-specify THRESHOLD - it's read from state.

**Example 3: Multiple SNAKE_CASE Keywords**
...
All SNAKE_CASE keywords are persisted. RESET command will clear them.
...
## Chart Layering and Positioning

Vizard supports Altair's composition operators through two keywords: **LAYER** and **PLOT** (with position).

### LAYER Keyword - Overlaying Charts

**Syntax:** `|| LAYER <natural_language_description>`

**Purpose:** Overlay a new chart layer onto the current chart using Altair's `+` operator.

**Encoding Inheritance:**
- Layers automatically inherit X, Y, and COLOR encodings from the base chart
- You can override inherited encodings by specifying new ones
- Natural language descriptions are interpreted by the LLM

**Common Layer Types:**
- **Regression lines:** "Least Square Line", "Linear fit", "Trend line"
- **Reference lines:** "Horizontal line at mean of Y", "Vertical line at X = 5"
- **Text labels:** "text", "text labels", "value labels"
- **Statistical overlays:** "Median line", "Confidence bands"

**Example 1: Bar chart with text labels**
```
...
line = points.mark_line(color='red').transform_regression('Horsepower', 'Miles_per_Gallon')
chart = (points + line).properties(width=600, height=400)
```

**Example 3: Horizontal reference line with styling**
```
|| PLOT scatter X date Y temperature COLOR city
|| LAYER Horizontal blue dotted line at mean of temperature
```
...
mean_temp = df.select(pl.col('temperature').mean()).item()
rule = alt.Chart(pl.DataFrame({'y': [mean_temp]})).mark_rule(
    strokeDash=[5, 5],
    color='blue'
).encode(y='y:Q')
...
### PLOT Keyword with Positioning - Multi-Panel Layouts

**Syntax:** `|| PLOT <chart_type> <encodings> <position>`

**Positions:**
- `above` - Place new chart above current (vertical: `&`)
- `below` - Place new chart below current (vertical: `&`)
- `left` - Place new chart to left of current (horizontal: `|`)
- `right` - Place new chart to right of current (horizontal: `|`)

**Axis Sharing Logic:**
- **above/below + no explicit X/Y:** Share X-axis (independent Y-axes)
- **left/right + no explicit X/Y:** Share Y-axis (independent X-axes)
- **Explicit X or Y:** Creates independent axes for that dimension

**Important:** `PLOT` keyword is REQUIRED for positioned charts.

**Example 1: Scatter with histogram below**
```
|| PLOT scatter X Horsepower Y Miles_per_Gallon
...
```
|| PLOT bar X gene Y expression TITLE Treated
|| PLOT bar X gene Y expression right TITLE Control
```

Generates:
```python
treated = alt.Chart(df).mark_bar(color='steelblue').encode(
    x=alt.X('gene:N', title='Gene'),
    y=alt.Y('expression:Q', title='Expression')
).properties(title='Treated', width=300, height=400)

control = alt.Chart(df).mark_bar(color='steelblue').encode(
    x=alt.X('gene:N', title='Gene'),
    y=alt.Y('expression:Q', title='Expression')
).properties(title='Control', width=300, height=400)

chart = treated | control
```

**Example 3: Complex combination (layers + positioning)**
```
|| PLOT scatter X Horsepower Y Miles_per_Gallon COLOR Origin
|| LAYER Least Square Line
|| PLOT histogram X below
```
...
Generates:
```python
# Base scatter with layers
scatter_base = alt.Chart(df).encode(
    x=alt.X('Horsepower:Q', title='Horsepower'),
    y=alt.Y('Miles_per_Gallon:Q', title='Miles Per Gallon'),
    color=alt.Color('Origin:N', title='Origin')
)
points = scatter_base.mark_point(size=60)
line = scatter_base.mark_line(color='red').transform_regression('Horsepower', 'Miles_per_Gallon')
scatter = (points + line).properties(width=600, height=400)

# Positioned histogram
histogram = alt.Chart(df).mark_bar().encode(
    x=alt.X('Horsepower:Q', bin=alt.Bin(maxbins=30), title='Horsepower'),
    y=alt.Y('count()', title='Count')
).properties(width=600, height=150)
...
**Syntax:** `|| LAYER line Y <new_column>`

**Purpose:** Overlay two Y variables on the same X-axis with independent scales.

**Implementation:** Use `.resolve_scale(y='independent')`
...
Generates:
```python
temp_line = alt.Chart(df).mark_line(color='red').encode(
    x=alt.X('date:T', title='Date'),
    y=alt.Y('temperature:Q', title='Temperature (°C)')
)
precip_line = alt.Chart(df).mark_line(color='blue').encode(
    x=alt.X('date:T'),
    y=alt.Y('precipitation:Q', title='Precipitation (mm)')
)
chart = (temp_line + precip_line).resolve_scale(y='independent').properties(width=600, height=400)
```

### State Management Across || Delimiters

Each `||` plotting section builds on the previous chart state:

1. **First ||:** Creates base chart
2. **Subsequent || LAYER:** Adds overlay using `+` operator
3. **Subsequent || PLOT <position>:** Positions chart using `&` or `|` operator

**State Flow Example:**
```
DATA cars
|| PLOT scatter X Horsepower Y Miles_per_Gallon    # Creates: base_chart
|| LAYER Least Square Line                          # Creates: base_chart + regression
|| PLOT histogram X below                           # Creates: (base_chart + regression) & histogram
```

**Implementation Note:** The chart variable accumulates composition across `||` delimiters.
...
- TEXT: Text labels
- LAYER: Overlay layer (LAYER text labels, LAYER Least Square Line)
- PLOT with position: above | below | left | right (PLOT histogram X below)
...
17. **Wrangling state management**:
    ...
    - All plotting keywords persist as usual
18. **Chart layering and positioning**:
    - **LAYER keyword** - Overlay charts using `+` operator:
      - Inherit X, Y, COLOR from current chart state
      - Parse natural language to determine mark type (text, line, rule, point)
      - Common patterns: "text labels" → mark_text, "Least Square Line" → transform_regression, "Horizontal line at mean of Y" → mark_rule with calculation
      - Generate appropriate mark_* method with styling from description (colors, strokeDash, etc.)
      - Accumulate: `chart = chart + layer`
    - **PLOT with position** - Positioned charts using `&` or `|`:
      - Parse position: above/below → `&`, left/right → `|`
      - Smart axis sharing: above/below shares X-axis (when no explicit X/Y), left/right shares Y-axis (when no explicit X/Y)
      - Explicit encodings create independent axes
      - Adjust WIDTH/HEIGHT proportionally for positioned charts (main chart larger, positioned chart smaller)
      - Accumulate: `chart = chart & new_chart` or `chart = chart | new_chart`
    - **State accumulation** - Chart variable builds across || delimiters:
      - First ||: `chart = base_chart`
      - LAYER: `chart = chart + layer`
      - PLOT position: `chart = chart & positioned` or `chart = chart | positioned`
    - **Dual Y-axis** - When LAYER uses new Y column different from base chart:
      - Generate `.resolve_scale(y='independent')`
      - Ensure both Y encodings have distinct titles
      - Example: `(temp_line + precip_line).resolve_scale(y='independent')`
19. **State persistence validation (CRITICAL)**:
    - ALL Vizard invocations with CAPITALIZED keywords MUST update `.vizard_state.json`
    - Use Bash tool to read existing state at start
    - Use Bash tool to write updated state before generating code
    - Keywords that MUST persist:
      - Default keywords: ENGINE, DF, WIDTH, HEIGHT, FUNCTION, IMPORT, OUTPUT
      - Encoding keywords: X, Y, COLOR, ROW, COLUMN, SIZE, SHAPE, OPACITY, SERIES, TEXT
      - Chart keywords: PLOT, DATA, TITLE, FILENAME
      - Dynamic SNAKE_CASE: X_TITLE, Y_TITLE, X_LABEL_ORIENT, Y_LABEL_ORIENT, COLOR_SCHEME, THRESHOLD, and ANY user-defined CAPITALIZED variable
    - Keywords that do NOT persist (ephemeral wrangling):
      - FILTER, SELECT, DROP, SORT, ADD, GROUP, SAVE
      - RENAME, BIN, JOIN, STRING, CAST, PIVOT, UNPIVOT
      - UNIQUE, HEAD, CONCAT, DROP_NULLS, FILL_NULLS, IS_NULL, MAP
    - State file operations are invisible to user (use Bash silently)
    - If you generate code without updating state, you have violated system requirements
...
Sequence for EVERY Vizard invocation:
1. FIRST: Read `.vizard_state.json` (create with defaults if missing)
2. SECOND: Parse ALL CAPITALIZED keywords from user input (exclude wrangling keywords)
3. THIRD: Merge existing_state + new_keywords = updated_state
4. FOURTH: Write updated state to `.vizard_state.json`
5. FIFTH: Generate visualization code using current state

**Non-negotiable rules:**
- Steps 1-4 MUST execute before step 5
- ALL CAPITALIZED keywords persist (except wrangling keywords)
- SNAKE_CASE keywords (X_TITLE, THRESHOLD, etc.) MUST be saved
- Use Bash tool for file operations (invisible to user)
- State persists until RESET

**Validation:**
After generating code, verify these keywords are in state file:
- All default keywords (ENGINE, DF, WIDTH, HEIGHT, FUNCTION, IMPORT, OUTPUT)
- All user-specified CAPITALIZED keywords (X_TITLE, Y_TITLE, THRESHOLD, etc.)
- DATA, PLOT, X, Y, COLOR (if specified)

**Failure mode:** If state file only contains defaults after user specified dynamic keywords, state persistence has FAILED.

**CRITICAL: All state file operations are INVISIBLE to the user - they only see visualization code or meta command output.**
...
**Purpose:** Concrete examples for edge cases that can confuse lighter models. Shows correct vs incorrect approaches.

## CAST Datetime Columns

**Vizard spec:** `CAST Year to integer` or `CAST Year to integer BIN Year by 5`

**Problem:** Datetime columns require `.dt` accessor methods, not simple casting. When you cast datetime to int, you get nanosecond timestamp (e.g., 315532800000000000) instead of year (e.g., 1975).

**CORRECT (datetime → extract year integer):**
```python
df = df.with_columns(pl.col('Year').dt.year().alias('Year'))
```

**WRONG (datetime → cast to int):**
```python
# ❌ DON'T DO THIS - produces nanosecond timestamp, not year integer
df = df.with_columns(pl.col('Year').cast(pl.Int64))
```

**Example in chain (STRING + CAST + BIN):**
```python
# Vizard: STRING uppercase Origin CAST Year to integer BIN Year by 5 as year_range
df = (df
    .with_columns(pl.col('Origin').str.to_uppercase())
    .with_columns(pl.col('Year').dt.year().alias('Year'))  # ✓ Use .dt.year()
    .with_columns(((pl.col('Year') // 5) * 5).alias('year_range')))
```

**Pattern:** For datetime columns → integer parts, use `.dt.year()`, `.dt.month()`, `.dt.day()`, etc.
For numeric/string → integer, use `.cast(pl.Int64)`.
