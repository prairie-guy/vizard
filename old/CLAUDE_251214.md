...
- Preprocessing keywords (FILTER, SELECT, DROP, SORT, ADD, GROUP, SAVE, RENAME, BIN, JOIN, STRING, CAST, PIVOT, UNPIVOT, UNIQUE, HEAD, CONCAT, MAP) are EPHEMERAL
...
- If ONLY preprocessing keywords present (FILTER, SELECT, DROP, SORT, ADD, GROUP, SAVE, RENAME, BIN, JOIN, STRING, CAST, PIVOT, UNPIVOT, UNIQUE, HEAD, CONCAT, MAP)
...
- **HELP** - Display help information (default: false)
  ...
  - Works for all Polars preprocessing keywords (FILTER, MAP, JOIN, etc.) and Altair visualization keywords (PLOT, X, Y, etc.)
